    ...
    public class DBQuery
    {
            ...
            try
            {
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        if (!string.IsNullOrEmpty(liPinMaItem.LiPinMa))
        ...
        public static List<InputKingPaiHangData> GetUserInputPaiHang(DBManager dbMgr, string startTime, string endTime, int maxPaiHang = 3)
        {
            ...
            try
            {
                ...
                // Vị trí này cần sử dụng timestamp theo chuẩn UNIX để định vị. Trong trường hợp thông thường, 
                // timestamp UNIX và inputtime là cùng một giá trị, tuy nhiên inputtime thường lớn hơn một chút so với UNIX.
                // Đối với việc nạp tiền của người dùng, sử dụng timestamp UNIX sẽ chính xác hơn. 
                // Ngoài ra, mặc định thông tin nạp tiền của người chơi thường chỉ có trong bảng t_inputlog, 
                // còn thông tin nạp tiền từ GM thì chỉ có trong bảng t_inputlog2.
                // Nếu có trường hợp người chơi xuất hiện trong cả hai bảng, thì người chơi đó có thể có hai dòng dữ liệu trong kết quả trả về. 
                // Khi đó, cần thực hiện thao tác lọc để loại bỏ dữ liệu trùng lặp.
                // Vì vậy, số dòng dữ liệu trả về tối đa sẽ nhân đôi, sau đó tiến hành lọc các người chơi trùng lặp trong dữ liệu kết quả.
        ...
        */
        ...
        public static RoleMiniInfo QueryRoleMiniInfo(long rid)
        {
            ...
            using (MyDbConnection3 conn = new MyDbConnection3())
            {
                ...
                using (MySqlDataReader reader = conn.ExecuteReader(cmdText))
                {
                    if (reader.Read())
                    {
